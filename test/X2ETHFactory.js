...
describe("X2ETHFactory", function () {
  ...
  it("setFunding", async () => {
    ...
    await expect(factory.connect(user0).setFunding(market.address, 499))
      .to.be.revertedWith("X2ETHMarket: funding range exceeded")

    await expect(factory.connect(user0).setFunding(market.address, 1000001))
      .to.be.revertedWith("X2ETHMarket: funding range exceeded")
    ...
    await factory.connect(user0).setFunding(market.address, 500)
    expect(await market.fundingDivisor()).eq(500)

    await factory.connect(user0).setFunding(market.address, 1000000)
    expect(await market.fundingDivisor()).eq(1000000)
  })
