...
describe("X2ETHMarket", function () {
  ...
  it("gas usage", async () => {
    ...
    const tx0 = await market.connect(user0).buy(bullToken.address, false, { value: expandDecimals(10, 18) })
    ...
    const tx1 = await market.connect(user0).buy(bullToken.address, false, { value: expandDecimals(10, 18) })
    await reportGasUsed(provider, tx1, "tx1 buy gas used")

    // user0 buys a bear to initialise the bear side
    const tx2 = await market.connect(user0).buy(bearToken.address, false, { value: expandDecimals(10, 18) })
    ...
    const tx3 = await market.connect(user1).buy(bearToken.address, false, { value: expandDecimals(10, 18) })
    ...
    const tx4 = await market.connect(user1).buy(bearToken.address, false, { value: expandDecimals(10, 18) })
    ...
    // first sell
    const tx5 = await market.connect(user0).sell(bullToken.address, expandDecimals(5, 29), user0.address, false)
    ...
    const tx6 = await market.connect(user1).sell(bearToken.address, expandDecimals(5, 29), user1.address, false)
    ...
    // higher costs for checking two prices
    const tx7 = await market.connect(user1).sell(bearToken.address, expandDecimals(5, 29), user1.address, false)
  ...
  it("sellWithoutDistribution", async () => {
    ...
    await expect(market.connect(user0).sell(bullToken.address, expandDecimals(1, 30), receiver0.address, false))
      .to.be.reverted
  ...
  it("distributeFees", async () => {
    ...
    await market.connect(user0).sell(bullToken.address, expandDecimals(1, 30), user0.address, false)
  ...
  it("distributeAppFees", async () => {
    ...
    await factory.connect(user3).setAppFee(market.address, 10, receiver3.address)

    await market.connect(user0).buy(bullToken.address, true, { value: expandDecimals(10, 18) })
    ...
    await market.connect(user1).buy(bearToken.address, true, { value: expandDecimals(10, 18) })
    ...
    expect(await market.interestReserve()).eq("0")
    await market.connect(user0).sell(bullToken.address, expandDecimals(5, 29), receiver0.address, true)
    ...
    await market.distributeAppFees()
